/*
 * jsp만으로 개발했던 방식에서 rsgist.jsp가 담당하는 업무를 서블릿으로 구현해본다 
 * */
...
public class RegistServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//클라이언트의 요청에 multipart/form-data 방식이 포함될 경우 텍스트가 아닌 바이너리 파일로 전송되는 것이다. 
		//따라서 업로드 컴포넌트를 사용해야 한다. 
		DiskFileItemFactory factory = new DiskFileItemFactory();
		ServletContext context = request.getServletContext();//어플리케이션과 관련된 정보를 담는 객체인 ServletContext를얻자
		...
		try {
			...
			for(FileItem item : items) {
				if (item.isFormField()) {//일반 텍스트 컴포넌트라면, 작성자,제목,내용들  textfield
					//vo에 알맞게 담자
					...
					board.setFilename(newName);//새로만들어진 이름을 vo담아야 insert문에 들어감
					//파일물리적으로 저장 
