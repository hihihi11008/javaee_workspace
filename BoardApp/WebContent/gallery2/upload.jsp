...
*/
   ...
   itemFactory.setRepository(new File(saveDir));   //저장될 위치 , 물리적인 저장이 아니라 임시디레게토리 
   ...
   for(FileItem item : items){   //int i=0; i<items.size(); i++
   		...
   		//반복문으로 처리되다보니, 파일만 따로 처리 하려면 , 구분로직이 필요함 
   		...
   		if(!item.isFormField()){ //텍스트박스가 아닌것만 업로드 처리 
		    //out.print("저는 "+item.getFieldName());
	   		...
	   		/*업로드처리하자 메모리상의 이미지 정보를 실제 물리적 파일로 저장하자 */
	   		//새롭게 생성할 파일명 
	   		//out.print(item.getName());//파일이름 추출
